...
<body>
	...
	try{
				...
				String str = "SELECT t.ticketID, t.class, p.flightID, p.price, f.departureTime, p.departureDate, d.airportName as departingFrom, f.arrivalTime, p.arrivalDate, a.airportName as arrivingAt FROM Ticket t join Trip p using(tripID) join Flight f using (flightID) join Route r using(routeID) join Airport d on r.departureAirportID = d.airportID join Airport a on r.arrivalAirportID = a.airportID WHERE t.username = \"" + newUsername + "\"";
				//Run the query against the database.
				ResultSet result = stmt.executeQuery(str);
			%>
				Flight reservations for <% out.println(newUsername);
		...
		//out.print(ex);
		out.println("Showing airport failed; Enter an airport ID that exists.");
	}
%>
